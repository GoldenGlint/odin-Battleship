...
test("accepts coordinates inside the board", () => {
    expect(testGameBoard.isValidCoordinate([0, 0])).toBe(true);
    expect(testGameBoard.isValidCoordinate([5, 5])).toBe(true);
    expect(testGameBoard.isValidCoordinate([9, 9])).toBe(true);
});

test("rejects coordinates outside the board", () => {
    expect(testGameBoard.isValidCoordinate([-1, 0])).toBe(false);
    expect(testGameBoard.isValidCoordinate([0, -1])).toBe(false);
    expect(testGameBoard.isValidCoordinate([10, 0])).toBe(false);
    expect(testGameBoard.isValidCoordinate([0, 10])).toBe(false);
});

test("receiveAttack rejects an out of bounds coordinate", () => {
    expect(testGameBoard.receiveAttack([10, 5])).toBe(false);

    expect(testGameBoard.hit).toEqual([]);
    expect(testGameBoard.miss).toEqual([]);
});

test("receiveAttack rejects negative coordinates", () => {
    expect(testGameBoard.receiveAttack([-1, 5])).toBe(false);
